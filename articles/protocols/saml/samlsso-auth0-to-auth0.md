---
description: Learn how to use SAML SSO with Auth0 as both the Service Provider and Identity Provider, using two Auth0 domains, allowing you to test your Auth0 SAML without configuring another provider to do so!
---

# SAML SSO with Auth0 as Service Provider and as an Identity Provider

This tutorial will create a simple example application that uses Auth0 to do SAML Single Sign On (SSO), using one Auth0 domain (domain 1) as a SAML Service Provider(SP), and authenticating users against a second Auth0 domain (domain 2) serving as SAML Identity Provider(IDP).  This gives you a way to test your Auth0 SAML domain (domain 1) configuration, using Auth0 as an IDP so you don't have to learn and set up another IDP.
...
1. [Establish two Auth0 domains](#1-establish-two-auth0-domains)
2. [Set up the Auth0 Identity Provider (IDP) (domain 2)](#2-set-up-the-auth0-idp-domain-2-)
3. [Set up the Auth0 Service Provider (SP) (domain 1)](#3-set-up-the-auth0-service-provider-domain-1-)
...
## 1. Establish two Auth0 Domains

If you do not already have two Auth0 domains, you will need to create them. If you do already have two domains, you can skip to step #2.

### In the Auth0 Dashboard

In the upper right corner, click on the name of your domain and in the popup menu which appears, select **+ Create Domain**.  

![](/media/articles/saml/samlsso-auth0-to-auth0/samlsso-auth0-01.png)

In the window which appears, enter a name for your second domain in the **"Your Auth0 domain"** field and press the **"SAVE"** button.

You can switch back and forth between the domains by going to the upper right corner of the dashboard, clicking on the name of the current domain, and using the popup menu which appears to switch between your domains.

## 2. Set up the Auth0 IDP (domain 2)

In this section you will configure one Auth0 domain (domain 2) to serve as an Identity Provider.  You will do this by registering a client, but in this case, the 'client' you register is really a representation of domain 1, the SAML Service Provider.
...
7. In the expanded window, scroll down to the **"Certificates"** section and click on the **"DOWNLOAD CERTIFICATE"** link and select PEM from the dropdown, to download a PEM-formatted certificate.  The certificate will be downloaded to a file called "${account.tenant}.pem".  Save this file as you will need to upload this file when configuring the other Auth0 domain, domain 1.

![](/media/articles/saml/samlsso-auth0-to-auth0/samlsso-auth0-03.png)

8. Click on the **"Endpoints"** tab and go to the **"SAML"** section.  Copy the entire contents of the **"SAML Protocol URL"** field and save it as in the next step you will need to paste it into the other Auth0 domain, domain 1.
...
## 3. Set up the Auth0 service provider (domain 1)

In this section you will configure another Auth0 domain (domain 1) so it knows how to communicate with the second Auth0 domain (domain 2) for single sign on via the SAML protocol.

Log out of **Domain 2** and log into **Domain 1**.
...
**Sign In URL:** enter the **"SAML Protocol URL"** field that you copied in section 2 above. (From domain 2 dashboard, Apps/APIs link, Settings tab, Advanced Settings, ENDPOINTS section, SAML tab, "SAML Protocol URL" field.)

**Sign Out URL:** enter the same URL as for the Sign In URL above.

**X509 Signing Certificate:**  
Click on the red **"UPLOAD CERTIFICATE..."** button and select the `.pem` file you downloaded from domain 2 in section 2 above.
...
In the window that appears, metadata about this SAML provider (domain 1) is displayed.  You will need to collect two pieces of information about this Auth0 domain (the service provider) that you will then paste into the other Auth0 domain you set up (the identity provider).
...
Once you go to that metadata URL, it will display the metadata for the Auth0 domain 1 (service provider side of the federation. It will look something like the following with your domain name in place of the 'xxxxx':
...
Copy and save this URL. This is the URL on domain 1 that will receive the SAML assertion from the IDP. In the next section you will give this URL to the IDP so it knows where to send the SAML assertion.

## 4. Add your Service Provider metadata to the Identity Provider

In this section you will go back and add some information about the Service Provider (domain 1) to the Identity Provider (domain 2) so the Identity Provider Auth0 domain knows how to receive and respond to SAML-based authentication requests from the Service Provider Auth0 domain.

* Log out of **Domain 1** and log back into **Domain 2**.

**In the Auth0 dashboard:** for Domain 2
...
That will trigger a login screen from domain 2, the Identity Provider.

Log in with the credentials for domain 2.
...
Make sure you are logged into the **Domain 1 Auth0 dashboard**.
...
In this section, you will test to make sure the SAML configuration between Auth0 domain 1 (Service Provider) and Auth0 domain 2 (Identity Provider) is working.
...
* You will first see a Lock login widget appear that is triggered by the Service Provider.  Enter the username of the test domain you created earlier.
...
The client ID for your client can be found in the **Auth0 dashboard** for **Domain 1** by going to "Clients"link and clicking on the "Settings" (gear) icon to the right of your client's name.
...
In this step, you will test your sample HTML application that uses the Auth0 SAML connection you set up in Domain 1 to perform SSO via SAML against Domain 2, serving as the SAML Identity Provider.
...
If sufficient time has passed, or if you delete your browser cookies before initiating the test, then you will be prompted to login when redirected to the Identity Provider.  Log in to the Identity Provider using the credentials for the test user you created in Auth0 Domain 2.
