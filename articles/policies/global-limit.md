...
This document describes the global rate limiting policy applicable to __free suscribers__. Please visit [Rate limits](/policies/rate-limits) for more information on other applicable limits.

## Limits
__Free suscribers__ Authentication API usage is restricted to __300 requests per minute__.

It is important to consider that the limit is global for the tenant and not per endpoint.
