...
::: panel Rate limits
Obtaining new tokens using the Refresh Token should occur only if the `id_token` has expired. There are rate limits in Auth0 that will throttle the amount of requests to this endpoint that can be executed using the same token from the same IP.
:::
