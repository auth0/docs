...
Auth0 API requires an `access_token`. You can get one by authenticating with your `client_id` and `client_secret` (It will be valid for 24 hours). To obtain the global client ID and global client secret see the **Advanced** tab under [Domain Settings](${manage_url}/#/domain/advanced) in the Auth0 dashboard.
