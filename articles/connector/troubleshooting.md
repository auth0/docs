...
This error applies to the AD/LDAP Connector in combination with the Auth0 Private SaaS.

When the connector will fail to start if unable to validate the SSL certificate configured in Private SaaS. This can happen when the Root Certificate (or any Intermediate Certificates) are missing in the machine's Certificate Store (Windows). In order to solve this you should import the certificate chain in the **Local Machine > Trusted Root** certificate store on the machine where the AD/LDAP Connector is installed.
