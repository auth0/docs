...
::: zoomable
![Centralized vs Embedded login flow](/media/articles/guides/login/centralized-embedded-flow.png)
:::
...
- **User Experience**: In the past, an argument could be made that the user experience with embedded login was better because it did not require redirecting users to another domain, which could be alerting. However, users are getting increasingly familiar with the process of being redirected to another domain to log in, mainly because they use social logins all the time. As a result, they don't find the process disruptive to their experience. Ask yourself, have you ever clicked **Login with Google** and get frustrated because you are redirected to Google's domain? You probably don't even notice it any longer.
...
For most situations, we recommend using a [Hosted Login Page (HLP)](/hosted-pages/login) to authenticate users. You can enable and customize your login page using the [Dashboard](${manage_url}/#/login_page).
