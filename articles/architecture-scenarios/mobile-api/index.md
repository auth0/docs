...
* The API is secured by ensuring that a valid <dfn data-key="Access Token">[Access Token](/tokens/overview-access-tokens)</dfn> is passed in the HTTP Authorization header when calls are made to the API (see [Implement the API](/architecture-scenarios/mobile-api/part-3#secure-the-endpoints))
