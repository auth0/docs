...
When calling one API from another API, cron jobs, report generators, or continuous integration/delivery systems, you will need a way to authorize the application instead of a user.  This requires some [machine-to-machine (m2m) authentication](#m2m-authentication).
