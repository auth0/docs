...
To enable authentication in your ASP.NET Core application, use the middleware provided by the SDK. Go to the  `Program` class and between `var builder = WebApplication.CreateBuilder(args);` and `var app = builder.Build();` call `var builder = WebApplication.CreateBuilder(args);` to configure the Auth0 ASP.NET Core SDK.
