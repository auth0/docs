...
With this change, access tokens generated by Auth0 will now have the correct audience value and can be validated properly by the backend API.
...
Once the token has been retrieved, it is used as a bearer token for the Authorization header in the call to `fetch`. The backend API will then validate this token and return either a `401 Unauthorized` response, or a `200` response with a JSON message. If a successful JSON message is received, then it is displayed on the screen for the user.
