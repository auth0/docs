...
> **Note:** This example assumes some kind of loading spinner is available in an `assets` directory. See the downloadable sample for a demonstration.
...
> **Note:** This example assumes you are using path-based routing which Angular defaults to. If you are using hash-based routing with `{ useHash: true }`, you won't be able to specify a dedicated callback route because the URL hash will be used to hold the user's authentication information.
