...
This page explains how to use version 2 and later of the Authorization Extension, [click here for documentation of version 1.](/authorization-extension-v1)
...
## Migration to v2 of the Authorization Extension

::: panel-warning Notice
One of the major changes of the v2 of the Authorization Extension is that the **Applications** section has been removed. This was done to remove complexity within the extension. There are many ways to do application level access. The recommended approach is to use rules and verify that a user has the right [role](#roles) or belongs to the right [group](#groups). 
:::
...
By creating a rule in your Auth0 account you can configure how the authorization extension has to behave during a login transaction. 

To configure these rules, click **Configuration** on the dropdown on the top right of the **Authorization Dashboard**.
...
You can import new data or export exisiting authorization data with a JSON file. This can be useful when moving over an environment, but rememmber roles and permissions are linked to specific clients, so you will need to update to the correct **applicationId** when moving environments.
