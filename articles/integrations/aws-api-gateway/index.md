...
## Introduction
This blog demonstrates how to use API gateway with IAM and a SAML provider. The approach enables you to tie permissions to  APIs you develop using the API gateway to your existing user base. It builds upon the [API gateway model walkthrough](http://docs.aws.amazon.com/apigateway/latest/developerguide/getting-started-models.html). 

### Prerequisite Setup Steps for AWS API Gateway
Perform the following steps in order to get create the lambda functions and apis for getting and putting pets from a DynamoDB table.
 - In the DynamoDB console, create a table “Pets” with a string hash key,   “username”.
 - Follow create the APIGatewayLamdaExecRole in [Walkthrough: Lambda Functions, step 4](http://docs.aws.amazon.com/apigateway/latest/developerguide/getting-started-models.html#getting-started-models-lambda), and expand the additional policy for dynamodb access table as shown below:
```js
{
    "Version": "2012-10-17",
    "Statement": [
...
```
 ...
 - Perform only section **To create the input model** in Step 1 of [Create Models of Walkthrough: Models and Mapping Templates](http://docs.aws.amazon.com/apigateway/latest/developerguide/getting-started-models.html). Name the model `PetModel`.
...
You should see an empty return result (`{}`).  You can go back to your GetPetInfo lambda, and click **Test** again. You should now see a single pet.
 - One more lambda is required that does nothing. This is used by the OPTIONS method for CORS described in the following section. Repeat the steps for creating a Lambda function and name it `NoOp`. For the code simply call `context.succeed('');`.
...
We’ll show how to achieve this with one AWS partner, Auth0, that offers a managed service for developers to integrate federation into their solutions. 
...
If you open up `apiClient.js`, you can see that the downloaded library has created wrappers for your API methods, like `petsPost` and `petsGet`.  Open `home.js`, and replace the contents of `callApi` with `alert('not implemented');` and rename `callApi` to `putPets`. Add a method for getting pets as follows:

```js
  function getPets() {
    // this is unauthenticated, so no secrets
    var apigClient = apigClientFactory.newClient({
        region: 'us-east-1' // OPTIONAL: Will default to us-east-1 if not set
    });

    var promise = apigClient.petsGet({});

    promise.then(function(response) {
      console.log(response);
      $scope.pets = response.body.pets;
      $scope.$apply();
    });

    promise.catch(function (response) {
      alert('retrieve pets failed');
...
```js
 function putPets(updatedPets) {
    ...
    var promise = apigClient.petsPost({},body);
    promise.then(function(response) {
      console.log(response);
    });
    promise.catch(function (response) {
  ...
  $scope.isAdmin = store.get('profile').connection === 'Username-Password-Authentication';
  $scope.adding = false;
...
Once the lambda function is defined, add another method, POST, to the `purchase` resource which calls the `PetPurchase` Lambda. Make sure you also add the `Access-Control-Allow-Origin` header with a value of `‘*’` to the POST method using the method response/integration response configuration. Optionally you can create an input model as well for the method request to enforce the inputs for the API, using this schema:
...
Go ahead and test the API gateway method, providing as input a message similar to this: 
...
First update the login logic to select different roles for different users. You can tell Auth0 which role to use when you get the delegation token by doing the select based upon whether or not the user is an admin.
...
<div class="home">
  ...
  <div ng-repeat="pet in pets track by pet.id">
    <div class="row">
        <div class="col-md-6 col-md-offset-3">
We have a {{pet.type}} for sale for {{pet.price}}
</div>
  ...
  <button class="btn sm-btn" ng-click="addPets()" ng-hide="adding || !isAdmin">
Add Pets
  </button>
  <div ng-show="adding">
        <label>Pet Type:&nbsp;</label>
        <input type="text" ng-model="newpet.type"></input><br />
        <label>Pet Price:&nbsp;</label>
 <input type="text" ng-model="newpet.price"></input><br />
        <button class="bnt sm-btn" ng-click="savePet()">save</button>
        <button class="bnt  sm-btn" ng-click="cancelAddPet()">cancel</button>
  <br /><br />
  </div>
...
.controller( 'HomeCtrl', function HomeController( $scope, auth, $http, $location, store ) {
 ...
 … 
  // following does some refactoring of previous putPets method, and adds
  // purchasing. 
  ...
  function putPets(updatedPets) {
    ...
    apigClient.petsPost({},{pets: updatedPets})
...
function (user, context, callback) {
  if(context.clientID === 'zKRCne6bQ8L1sGqfvRfOvBJYBCj1XCYC') {    
    var socialRoleInfo = {
