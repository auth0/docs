...
As explained in the /userinfo endpoint docs entry, /userinfo obtains information using the Management API and therefore requires an [Access Token](/tokens/get-access-tokens) (obtained during login) instead of the [ID Token](/tokens/id-token) used by /tokeninfo.
