...
::: note
In the example that follows, we use the [Single-Page Login Flow](/flows/guides/single-page-login-flow/call-api-using-single-page-login-flow), the recommended OAuth 2.0 flow for client-side apps). You can get Access Tokens though for any application type (see [Get Access Tokens](/tokens/guides/access-token/get-access-tokens).
:::
