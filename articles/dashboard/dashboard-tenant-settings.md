...
Allows you to specify the **SSO Cookie Timeout**. This value is the maximum length of the login session lifetime, which is how long the session will stay valid measured in minutes. The default value is 10080 minutes (or 7 days).
...
Auth0 also sets a value for Idle Timeout, which is the allowed duration of inactivity for a session before a new session is required. Currently, this value is set at 7 days and is not configurable. If the **SSO Cookie Timeout** is set to longer than the Idle Timeout, then if the session is not used within 7 days, the session will expire and a new session must be created.
