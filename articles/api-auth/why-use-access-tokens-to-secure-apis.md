...
OpenID Connect issues an identity token, known as an `id_token`, while OAuth 2.0 issues an `Access Token`.
...
::: note
  Access Tokens should be treated as opaque strings by applications. They are only meant for the API. Your application should not attempt to decode them or depend on a particular <code>Access Token</code> format.
:::
