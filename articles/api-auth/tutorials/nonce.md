...
# Mitigate replay attacks when using the Single-Page Login Flow

<%= include('../../_includes/_pipeline2') %>

To mitigate replay attacks when using the [Single-Page Login Flow](/flows/concepts/single-page-login-flow), a [cryptographic nonce](https://en.wikipedia.org/wiki/Cryptographic_nonce) must be sent on authentication requests [as required by the OpenID Connect specification](https://openid.net/specs/openid-connect-core-1_0.html#ImplicitAuthRequest).
...
For more information on where to include the nonce, see [Call API Using the Single-Page Login Flow](/flows/guides/single-page-login-flow/call-api-using-single-page-login-flow).
