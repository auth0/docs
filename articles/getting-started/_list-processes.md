...
<% } else if (screen === "connections") { %>
  ...
  - [Passwordless](${manage_url}/#/connections/passwordless): let your users signup and login using one-time codes (delivered by email or SMS) or one-click links, instead of passwords. [More info on <dfn data-key="passwordless">[Passwordless](/connections/passwordless)</dfn>.
