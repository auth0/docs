...
If using [Lock](/libraries/lock), this is as simple as initiating Lock with the following option:
...
* You could use non-human-readable connection names and use some external mechanism to map these to users (such as through a primary verification, out of band channel for example).
...
[Lock](/libraries/lock) has built in functionality for identity provider selection. For social connections it will show logos for all those enabled in that particular app.
