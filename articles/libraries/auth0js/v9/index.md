...
| `state` | optional | (String)  An arbitrary value that should be mantained across redirects. It is useful to mitigate CSRF attacks and for any contextual information, such as a return URL, that you might need after the authentication process is finished (See the [state documentation](/docs/protocols/oauth2/oauth-state) for more information). |
