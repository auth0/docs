...
* `'openid'`: It will return, not only the Access Token, but also an `id_token` which is a JSON Web Token (JWT). The JWT will only contain the user id (sub claim). You can use objc constant `A0ScopeOpenId`.
...
Also when need to keep the `id_token` alive, you can request a Refresh Token adding to the scope the value `offline_access` (Or use the constant `A0ScopeOfflineAccess`).
