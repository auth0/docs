---
url: /private-saas/geo-ha
section: private-saas
description: Descriptions of Private SaaS Geo HA
---

# Auth0 Private SaaS: High Availability Geo Cluster (Geo HA)

The high availability geo cluster is an Private SaaS implementation that provides data center redundancy and automatic failure handling. This is the highest form of Private SaaS availability offered by Auth0.
...
![](/media/articles/private-saas/geo-ha.png)
...
* one standby data center with Private SaaS;
* one arbiter, a seventh instance that is located in its own data center.

The standby data center instances possess the same Private SaaS configuration as the primary data center instances, and continuous synchronization ensure that the data on the primary and standby data centers mirror each other.
...
Auth0 requires the use of a geographically-aware load balancer or DNS failover solution that prefers to serve application requests using the primary data center, despite the fact that the Private SaaS in the hot standby data center are active and able to serve the requests.
...
As long as all instances are visible to each other, the primary data center will always be elected. If the data center fails (that is, the instances are *not* visible to the witness and Private SaaS in the standby data center), then a node in the secondary data center will be elected to become primary.
...
3. **The Private SaaS associated with the primary data center begin failing.** Because the primary data center instances cannot communicate with the instances in the standby data center, the associated Private SaaS begin to fail.
...
5. **The Private SaaS associated with the standby data center are now serving requests** and acting as the primary data node due to its election in step 2.

### Further Reading

* [Geographic High-Availability Private SaaS Failure Scenarios and Testing](/private-saas/geo-ha/disaster-recovery)
