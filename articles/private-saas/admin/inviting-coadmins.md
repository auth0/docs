...
description: How to invite additional administrators to your Private SaaS implementation
---

# Private SaaS Administration: Inviting Co-Administrators

You may invite additional users to become co-administrators of your Private SaaS implementation. This is done via the Private SaaS configuration area of the Management Dashboard.

> Administrators for Private SaaS instances are authenticated with the root tenant authority (RTA, the primary tenant, sometimes called the **config** tenant). The Dashboard is represented by the **"Auth0"** app/client in the RTA account. By default the RTA has a DB connection enabled, **"Initial-Connection"**, that is used to authenticate Dashboard users.
Since that connection has signups disabled, users will be have to be created beforehand.

[![](/media/articles/private-saas/admin/invite-co-admins.png)](https://auth0-1.wistia.com/medias/2t8n98qc5j)
...
2. Switch over to the App Tenant so that you will have access to Private SaaS configuration options.
...
At this point, if you have SMTP configured on Private SaaS, the user will receive an email inviting them to log in as an administrator. They will need to use the email and password used in step 1.
