...
Using the user’s unique `user_id` from their Auth0 [user profile](/user-profile/normalized), we can make sure the data is associated with the user in our separate database in order to query by a unique identifier. Here is an example row from the `songs` table in our Heroku Postgres database:
...
![](/media/articles/tutorials/data-scenarios/home.png)
...
![](/media/articles/tutorials/data-scenarios/settings.png)
