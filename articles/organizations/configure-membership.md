...
To assign a member via the Auth0 Dashboard:
...
| `403` | `insufficient_scope` | Insufficient scope; expected any of: `create:organizations_members`. | Tried to read/write a field that is not allowed with provided bearer token scopes. |
...
To invite a user via the Auth0 Dashboard:
...
https://auth0team.atlassian.net/wiki/spaces/USER/pages/1000735499/User+Invitation
...
To assign a role to an organization member via the Auth0 Dashboard:
